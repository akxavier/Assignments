...
features = ['x1', 'x2', 'x3']
for dim in range(3):
    print("\nClassifying using feature vector ", features[:dim+1])
    w = [w1[:, 0:dim+1], w2[:, 0:dim+1]]
    ...
    for i in range(class_count):
        sigma[i] = np.zeros((dim+1, dim+1))
        for j in range(n):
            sigma[i] += np.outer(w[i][j] - mu[i], w[i][j] - mu[i])
        sigma[i] /= (n - 1)

    for point in points:
        g1_x = calc_gradient(point[:dim+1], mu[0], sigma[0], priors[0])
        g2_x = calc_gradient(point[:dim+1], mu[1], sigma[1], priors[1])
        if g1_x > g2_x:
            print("    Point ", point, " belongs to class w1")
        else:
            print("    Point ", point, " belongs to class w2")
