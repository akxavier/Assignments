...
void Update(struct tree *T, int k) {
	node x = Search(T, k);
	x -> stat = 'Y';
}

int Not_Vaccinated(node x) {
	if (x == NULL)
		return 0;
	...
	return (sum + Not_Vaccinated(x -> left) + Not_Vaccinated(x -> right));
}

int Vaccinated(node x) {
	if (x == NULL)
		return 0;
	...
	return (sum + Vaccinated(x -> left) + Vaccinated(x -> right));
}
		

void Find_Diff(struct tree *T, int k) {
	node x = Search(T, k);

	int n = Not_Vaccinated(x->left);
	int y = Vaccinated(x->left);

	int diff = (n > y) ? (n - y) : (y - n);
	printf("%d\n", diff);
}
...
void Print(node x) {
	if (x == NULL) {
		...
		printf ("%d ", x -> key);
		printf ("%d %d ", Not_Vaccinated(x->left), Vaccinated(x->left));
		Print(x -> left);
		Print(x -> right);
...
void Print_Tree(struct tree *T, int k) {
	node x = Search(T, k);
	Print(x);
}
